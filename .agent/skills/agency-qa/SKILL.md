...
description: >
  ...
  REVIEW — or when the coordinator routes a review task during the ARCHITECTURE
...
- **Sign-off** — Once all CRITICAL/HIGH/MEDIUM issues are resolved, write a clear sign-off statement and post `APPROVE` to `agency-coordinator`.

### When Done (All Issues Resolved)

Post `APPROVE` to `agency-coordinator` in `inbox.jsonl`. This is the final gate. The CEO cannot enter DELIVERY without this message.
